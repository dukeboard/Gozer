...
public class GozerVersionResolver
        implements VersionResolver, Service {

    private static final String MAVEN_METADATA_XML = "maven-metadata.xml";
    private static final String GOZER_METADATA_XML = "gozer-metadata.zip";


    private static final GozerRepositoryLayout gozerLayout = new GozerRepositoryLayout();
    ...
    public GozerVersionResolver setLogger(Logger logger) {
        this.logger = (logger != null) ? logger : NullLogger.INSTANCE;
        return this;
    }

    public GozerVersionResolver setMetadataResolver(MetadataResolver metadataResolver) {
    ...
    public GozerVersionResolver setSyncContextFactory(SyncContextFactory syncContextFactory) {
    ...
    public VersionResult resolveVersion(RepositorySystemSession session, VersionRequest request)
        ...
        if (metadata == null) {
            ...
            RemoteRepository gozerRepoFound = null;
            for (RemoteRepository repository : request.getRepositories()) {
                if (repository.getUrl().startsWith("gozer")) {
                    gozerRepoFound = repository;
                }
            }

            if (gozerRepoFound != null) {
                String gozer = gozerRepoFound.getUrl().substring(6);
                if(!gozer.endsWith("/")){
                    gozer = gozer+"/";
                }
                gozer = gozer+gozerLayout.getPath(metadata);
                URLConnection gozerConnection = null;
                try {
                    URL gozerURL = new URL(gozer);
                    gozerConnection = gozerURL.openConnection();
                    GozerFileHelper.unzipToTempDir(gozerConnection.getInputStream(),session.getLocalRepository().getBasedir());
                } catch (Exception e) {
                    e.printStackTrace();
                } finally {
