    ...
    implements RepositoryConnector
{
    ...
    private Boolean useGozer = false;
        ...
        throws NoRepositoryConnectorException
    {
        ...
        System.out.println("Hello Connector "+repository.getProtocol());
        ...
        if ( !repository.getProtocol().regionMatches( true, 0, "http", 0, "http".length() ) &&
            ...
            !repository.getProtocol().regionMatches( true, 0, "gozer", 0, "gozer".length() )
           )
        {
            throw new NoRepositoryConnectorException( repository );
        }

        if(repository.getProtocol().regionMatches( true, 0, "gozer", 0, "gozer".length() )){
            useGozer = true;
        }
        ...
        for ( ArtifactDownload download : artifactDownloads )
        {
            ...
            GetTask<?> task =
                new GetTask<ArtifactTransfer>( resource, download.isExistenceCheck() ? null : download.getFile(),
                                               download.getChecksumPolicy(), latch, download, ARTIFACT, true,download.getRepositories() );
            tasks.add( task );
            task.run();
        }
        ...
        implements Runnable
    {
        ...
        private List<RemoteRepository> cachedRepos = null;

        public GetTask( String path, File file, String checksumPolicy, CountDownLatch latch, T download,
                        ExceptionWrapper<T> wrapper, boolean allowResumable , List<RemoteRepository> repos)
        {
